...
export default function FormCanvas({
  ...
  const themeStyles = useMemo(() => {
    ...
    const buttons =
      typeof currentTheme.buttons === "string"
        ? JSON.parse(currentTheme.buttons)
        : currentTheme.buttons;

    const inputFields =
      typeof currentTheme.inputFields === "string"
        ? JSON.parse(currentTheme.inputFields)
        : currentTheme.inputFields;
    ...
    // Input sizes
    const inputSizes: Record<string, { height: string; fontSize: string }> = {
      sm: { height: "32px", fontSize: "13px" },
      md: { height: "40px", fontSize: "14px" },
      lg: { height: "48px", fontSize: "16px" },
    };

    const buttonSize = buttonSizes[buttons?.size || "md"];
    const inputSize = inputSizes[inputFields?.size || "md"];
    ...
    return {
      ...
      // Buttons
      buttonRadius: buttonRadii[buttons?.style || "rounded"],
      buttonHeight: buttonSize.height,
      buttonPadding: buttonSize.padding,
      buttonFontSize: buttonSize.fontSize,
      buttonVariant: buttons?.variant || "solid",
      // Input Fields
      inputStyle: inputFields?.style || "outlined",
      inputHeight: inputSize.height,
      inputFontSize: inputSize.fontSize,
    };
  }, [currentTheme]);
  ...
  useEffect(() => {
    ...
    if (currentTheme.colors) {
      ...
      // Set both --theme-* and --preview-* for compatibility
      ...
      // Also set --preview-* variables
      root.style.setProperty("--preview-primary", colors.primary || "#6366f1");
      root.style.setProperty(
        "--preview-background",
        colors.background || "#ffffff"
      );
      root.style.setProperty("--preview-card", colors.card || "#ffffff");
      root.style.setProperty("--preview-text", colors.text || "#0f172a");
      root.style.setProperty(
        "--preview-text-muted",
        colors.textMuted || "#64748b"
      );
      root.style.setProperty("--preview-border", colors.border || "#e2e8f0");
      root.style.setProperty("--preview-accent", colors.accent || "#eef2ff");
    }
    ...
    if (currentTheme.typography) {
      ...
      // Also set --font-heading and --font-body for global usage
      root.style.setProperty(
        "--font-heading",
        `"${typography.headingFont || "Sora"}", sans-serif`
      );
      root.style.setProperty(
        "--font-body",
    ...
    if (currentTheme.layout) {
      ...
      root.style.setProperty(
        "--preview-spacing",
        spacings[layout.spacing] || "1rem"
      );
      root.style.setProperty(
        "--preview-shadow",
        shadows[layout.shadow] || shadows.md
      );
    }
    ...
    if (currentTheme.buttons) {
      ...
      const size = buttonSizes[buttons.size || "md"];
      root.style.setProperty(
        "--preview-button-radius",
        buttonRadii[buttons.style] || "8px"
      );
      root.style.setProperty("--preview-button-height", size.height);
      root.style.setProperty("--preview-button-padding", size.padding);
      root.style.setProperty("--preview-button-font-size", size.fontSize);
      root.style.setProperty(
    ...
    if (currentTheme.inputFields) {
      ...
      const size = inputSizes[inputFields.size || "md"];
      root.style.setProperty(
        "--preview-input-style",
        inputFields.style || "outlined"
      );
      root.style.setProperty("--preview-input-height", size.height);
      root.style.setProperty("--preview-input-font-size", size.fontSize);
    }
    ...
    return () => {
      [
        ...
        "--preview-shadow",
        "--preview-button-radius",
        "--preview-button-height",
        "--preview-button-padding",
        "--preview-button-font-size",
        "--preview-button-variant",
        "--preview-input-style",
        "--preview-input-height",
        "--preview-input-font-size",
        "--font-heading",
        "--font-body",
  ...
  return (
    <div className="h-full overflow-hidden">
      <div className="relative h-full flex">
          ...
          style={
            themeStyles
              ? ({
                  ...
                  "--preview-radius": themeStyles.borderRadius,
                  "--preview-spacing": themeStyles.spacing,
                  "--preview-shadow": themeStyles.shadow,
                  "--preview-button-radius": themeStyles.buttonRadius,
                  "--preview-button-height": themeStyles.buttonHeight,
                  "--preview-button-padding": themeStyles.buttonPadding,
                  "--preview-button-font-size": themeStyles.buttonFontSize,
                  "--preview-input-height": themeStyles.inputHeight,
                  "--preview-input-font-size": themeStyles.inputFontSize,
        ...
        >
          <div className="px-3 sm:px-4 lg:px-6 pt-20 sm:pt-24 pb-24 sm:pb-8">
            <div
              ...
              style={{ gap: themeStyles?.spacing || "1.5rem" }}
            >
              {/* Form Header */}
              <FormHeader
        ...
        {/* Mobile Overlay */}
        <MobileOverlay
          isVisible={leftSidebarOpen || rightSidebarOpen}
